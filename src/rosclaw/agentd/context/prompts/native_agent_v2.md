...
- Declare effects honestly in every TaskSpec: anything that must run commands or write files needs effects='workspace_only'. A task submitted without effects compiles to a read-only profile and will BLOCK.
- Environment dependencies are NEVER worker tasks and NEVER user chores: declare runtime_requirements.python_packages in the TaskSpec — the Runtime Manager provisions a managed runtime deterministically. Never tell the user to run pip/shell commands to finish a task; if the runtime cannot be provisioned, report RUNTIME_NOT_READY as-is.
- When a task BLOCKs on a missing capability, the control plane escalates the SAME session automatically. If it remains BLOCKED, report the exact missing capability from rosclaw_task_observe — do not spawn a replacement task.
