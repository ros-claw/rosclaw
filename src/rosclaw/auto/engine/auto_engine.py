...
class AutoEngine:
    ...
    def run_experiment(self, experiment: ExperimentSpec, runner: str = "local") -> dict:
        ...
        r = {
            "local": self.local_runner,
            "sandbox": self.sandbox_runner,
            "darwin": self.darwin_runner,
        }.get(runner, self.local_runner)
    ...
    def generate_report(self, task_id: str) -> EvolutionReport:
        ...
        experiments = len(
            [
                e
                for e in self._iterate("experiments")
    ...
    def run(self, task_id: str, rounds: int = 10, dry_run: bool = False) -> EvolutionReport:
        ...
        for i in range(rounds):
            ...
            search_space = (
                diag.recommended_search_space
                if diag.recommended_search_space
                else {"param_range": [0.0, 1.0]}
            )
            ...
            exp = self.create_experiment(
                ...
                task.name,
                task.target_skill_id,
                f"{task.target_skill_id}_candidate",
            )

            raw_local = self.run_experiment(exp, runner="local")
            if not raw_local["success"]:
                self.register_deadend(
                    task_id, f"round_{i}", "Local smoke test failed", [raw_local.get("error", "")]
                )
                continue
            ...
            if not raw_darwin["success"]:
                self.register_deadend(
                    task_id, f"round_{i}", "Darwin benchmark failed", [raw_darwin.get("error", "")]
                )
                continue
