...
| ROS 2 Turtlesim 受控运动 | **未验证** | Connector 契约已存在，但当前验证环境尚未运行 ROS 2 黄金路径。 |
...
| 阶段 | 重点 |
|---|---|
| **当前 / Alpha** | rosclawd 进程边界与持久化本地控制账本、可信 Action/Receipt 契约、Fixture/Provider/Driver 失败关闭、MuJoCo UR5e Reach 黄金闭环、分级 Doctor。 |
| **下一步** | 所有物理副作用迁移至 Gateway；账本压缩与外部见证；取消/抢占；ROS 2 Turtlesim 真运动观测闭环。 |
