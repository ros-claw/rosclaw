...
| rosclawd Agent/物理执行边界 | **组件与系统已验证** | 独立进程与 clean-wheel 跨 UID socket、固定的 SO_PEERCRED 身份、Agent 不可读写的 daemon 私有状态、精确 Permit 和持久重启恢复已通过系统测试，并包含参考 systemd sandbox；同所有者协同回滚、站点设备/凭证 ACL、SROS2 与真机验收仍待完成。 |
| Action Contract / Gateway | **组件与系统已验证** | 版本化 Action/Receipt、证据等级、幂等、Body 独占租约和 Executor 缺失时失败关闭。 |
...
| RealSense 只读感知路径 | **实验性** | 已签名且锁定依赖 commit 的 RealSense Robot Pack 已覆盖 discover/add/configure/verify、Body 绑定、daemon 侧 RGB-D 执行、Artifact 哈希和如实的 H3 候选证据；仓库中仍无独立验证的真机采集 Run。 |
| RH56 LeRobot 单步闭环 | **开发者已观察；独立复验中** | SerialModbusTransport、Shadow 读取、分级 REAL 动作和故障注入已有开发者真机观察；独立 v1 真机复验与 Agent 黑盒仍未完成。 |
...
队列、物理 Runtime、Driver、E-Stop Latch 和 Receipt。经过认证的本地账本会
持久化 Permit 消耗和 Action 状态迁移，在重启后恢复终态 Receipt；若 REAL
动作执行中断，则失败关闭并等待 daemon UID 操作员复核。
...
Robot Pack 是位于 Body、Hardware MCP、Capability、Policy、Calibration 和
Verification 等单项资产之上的签名、版本化硬件接入单元。首个内置 Pack
支持只读感知型 RealSense D405/D435i：
...
只有当当前主机需要安装该 Pack 锁定 commit 的原生 MCP 依赖时，才增加
