...
> ⚠️ **项目状态：Pre-Alpha (V0.1)**
>
> ROSClaw 目前正在积极开发中。我们不是在构建另一个简单的 "LLM-to-ROS2 API 封装器"（那种东西已经很多了）。我们正在构建一个完整的**具身 AI 操作系统**，具有基于 MuJoCo 的数字孪生防火墙、用于多智能体协调的反射握手协议，以及自动 EAP 数据飞轮。
>
> 当前版本是一个结构性概念验证。生产级的 ROS 2 Action 服务器、MoveIt 2 集成，以及完整的 e-URDF 防火墙将在即将发布的 `v0.2.0` 里程碑中推出。敬请期待。
...
### 💡 ROSClaw 中的 "ROS" 意味着什么？

我们超越了传统中间件。在我们的架构中，数字世界中的代理简单地称为 "Agent"（通过 OpenClaw）。**当该代理进入物理世界时，它被称为 "Robot"（机器人）。**

ROSClaw 是促成这一转变的**通用具身操作系统**。我们不仅仅是另一个 "LLM-to-ROS2 API 封装器"。虽然传统框架（如那些与 *ROSClaw* 同名的学术论文）纯粹专注于将文本翻译成 ROS 2 主题，但我们将传统的 ROS 1、ROS 2 和 IoT 协议仅仅视为"南向驱动"。

我们的核心价值在于 **OS 内核**：语义-物理防火墙、异步脑-小脑路由，以及自主数据飞轮。我们是一个"超级插件"，无缝地将整个物理生态系统连接到任何多模态代理。
