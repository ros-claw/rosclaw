...
| rosclawd Agent/物理执行边界 | **组件与系统已验证** | 独立进程 socket、SO_PEERCRED、精确 Permit，以及用于持久化 Permit 消耗、重启恢复 Action/Receipt 和中断 REAL 动作人工复核的 HMAC 链式 SQLite 账本已通过系统测试；同所有者协同回滚、生产跨 UID、SROS2 与真机验收仍待完成。 |
...
同 UID 开发模式不构成硬件权限边界。REAL 部署必须采用独立用户的 systemd
配置、设备 ACL、凭证隔离及 SROS2/DDS 权限控制，详见
