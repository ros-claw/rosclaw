...
class TestSerialDriverExtended:
    def test_build_move_packet(self):
        driver = SerialDriver("test", joint_dof=6, fixture_mode=True)
    ...
    def test_build_gripper_packet(self):
        driver = SerialDriver("test", fixture_mode=True)
    ...
    def test_parse_state_response_valid(self):
        driver = SerialDriver("test", joint_dof=3, fixture_mode=True)
        fmt = "<B3f3f3f"
    ...
    def test_set_gripper_fixture(self):
        driver = SerialDriver("test", fixture_mode=True)
        driver.initialize()
        driver.start()
