"""Pytest wrapper for multi-node concurrency tests."""
...
def test_ros2_multi_node_concurrency():
    ...
    result = subprocess.run(
        [sys.executable, "scripts/legacy/ros2_rclpy/test_ros2_multi_node.py"],
        capture_output=True,
        text=True,
        cwd=repo_root(),
    ...
    if result.stderr:
        print(result.stderr)
    assert result.returncode == 0, f"Multi-node tests failed:\n{result.stdout}\n{result.stderr}"
