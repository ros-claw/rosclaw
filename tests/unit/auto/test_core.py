...
def test_auto_task_roundtrip():
    t = AutoTask(
        ...
        robot_id="panda",
        environment_id="maniskill",
        target_skill_id="pick_v1",
    )
    d = t.to_dict()
...
def test_experiment():
    e = ExperimentSpec(
        ...
        proposal_id="p1",
        patch_id="patch1",
        baseline_skill_id="pick_v1",
        candidate_skill_id="pick_v1_c",
    )
...
def test_champion():
    c = Champion(
        id="c1", skill_id="pick_v2", task_id="t1", level="sim", metrics={"success_rate": 0.76}
    )
    assert c.level == "sim"
...
def test_deadend():
    d = DeadEnd(
        id="de1", task_id="t1", direction="scale > 0.8", rejection_reason="collision spikes"
    )
...
def test_champion_card_markdown():
    card = ChampionCard(
        skill_id="pick_v2", previous_champion="pick_v1", improvement={"success_rate": "0.58 → 0.76"}
    )
    md = card.to_markdown()
