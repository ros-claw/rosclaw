...
from rosclaw.evolution.orchestrator.runners import (
    DarwinRunner,
    LocalRunner,
    RunnerResult,
    SandboxRunner,
...
def test_local_runner_simulated_experiment_with_patch():
    ...
    exp = ExperimentSpec(
        ...
        candidate_skill_id="pick_v1_candidate",
...
def test_local_runner_simulated_experiment_no_patch():
    ...
    assert (
        abs(
            result.metrics["candidate"]["success_rate"] - result.metrics["baseline"]["success_rate"]
        )
        < 0.1
    )
