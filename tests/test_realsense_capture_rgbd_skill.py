    ...
    script.write_text(
        "\n".join(
            [
                ...
                "            text = json.dumps({'status': 'started', 'serial': args.get('serial')})",
                "            send({'jsonrpc': '2.0', 'id': req_id, 'result': {'content': [{'type': 'text', 'text': text}]}})",
                "        elif name == 'capture_aligned_rgbd':",
...
@pytest.fixture
def workspace_with_realsense_body(
    tmp_path: Path, fake_mcp_script: Path, fake_ros_mcp_script: Path
) -> Path:
