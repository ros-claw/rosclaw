...
Status: implemented (PR-MEM-3 / PR-SDB-2 / PR-MEM-4), validated on real RH56 hardware and a real SeekDB Server.
...
Outbox / Projection (idempotent, rebuildable)
        ↓
Versioned SeekDB collections (manual embeddings, analyzer per build)
        ↓
...
- BOTH hybrid legs (BM25 + KNN) receive the SAME hard metadata filter — cross-robot/body/tenant leakage = 0 (measured on embedded + server engines);
...
| **qwen3_06b_768_v1** | same | 768 | **production pick** (bake-off winner, §1.2 downsize rule) |
| qwen3_06b_512_v1 | same | 512 | fallback |

Jetson local inference measured: ~103 ms/doc FP16 batch, 1.25 GB VRAM, load 0.7 s warm. Qwen query instruction (English, per model card) on queries only; Matryoshka truncation + renormalization for 768/512.
...
→ benchmark
→ activate (atomic flip: ACTIVE→OLD, READY→ACTIVE)
→ keep OLD for rollback (registry flip, no rebuild)
```

Physical names embed the profile: `memory_items__qwen3_06b_1024_v1__ik`. Collections are NEVER shared across models/dimensions (§17.5); a GTE vector can never query a Qwen index.

Analyzers on the server: `ik` and `ngram` both built; CJK probes tie on the current corpus (9/9 both); `ik` ACTIVE. Final analyzer decision defers to the bake-off with the growing corpus.

## 7. Reranker

`embedding/reranker.py` — Qwen3-Reranker-0.6B @e61197ed cross-encoder.

High-risk paths only: failure recovery, HOW rule selection, safety-adjacent history, hard negatives (middle vs thumb_rot). Candidate window 20 → final 5. Measured on 105 high-risk benchmark queries: base R@1 0.8286 → +reranker 0.9143 (**+8.6pp**, 9 better / 0 worse) at ~266 ms/query — justified for recovery decisions, never on the hot path.

## 8. Degradation chain (§11)

```text
Qwen provider available   → native hybrid (manual embeddings)
provider unavailable      → SeekDB BM25 + metadata filter (EmbeddingUnavailableError, logged)
