...
| G41 GPU Live Qualification | 真实 GPU 上 jax-cuda 跑 MJX/MJWarp 资格 | GB10 aarch64 实测：jax 0.10.2 cuda12（官方 aarch64 wheel）+ mujoco-mjx 3.13.0，mjx.step GPU 真跑——`docs/reports/gpu-mjx/01_live_qualification.md` + JSON 工件 + `tests/sim/test_mjx_live.py`（CI 无 GPU 诚实 skip） | ✅ PASS（2026-09-23 live，MJX；MJWarp 仍 NOT_RUN） |
| G42 GPU/CPU Semantic Agreement | final qpos/qvel + 轨迹检查点 + task_success + peak contact force 全语义比对；分歧分类落 counterexample 语料 | `tests/sim/test_gpu_semantic.py`（门逻辑）+ **live**：tiny_arm 伺服跟踪 100 步 MJX vs CPU 零分歧 SEMANTIC_AGREEMENT（x64 对齐，1e-6 容差）→ promoted（`docs/reports/gpu-mjx/live_qualification_2026-09-23.json`） | ✅ PASS（MH24 门逻辑 + 2026-09-23 live；peak force 通道未接记 None） |
| G43 ROS2 Exact-step Agreement | 真桥 exact-step 与 sim 一致性 | GB10 实测（2026-09-23）：ros-jazzy-mujoco-ros2-control 0.1.1（tuna 镜像装通）headless 起桥 → pause → reset_world(keyframe) → step_simulation(500) → /joint_states vs 本地 MuJoCo 3.13.0 对照 rollout——qpos/qvel 终态**逐位一致**（diff 0.0/0.0；桥 3.12.0 vs 本地 3.13.0 跨版本）；`scripts/ros2_bridge_live.py` + `tests/sim/test_ros2_bridge_live.py`（CI 无桥诚实 NOT_RUN 锁定）+ 证据 `docs/reports/ros2-bridge/01_exact_step_agreement.json` | ✅ PASS（2026-09-23 live；单摆单关节模型——更广模型矩阵留后续） |
