...
├── sessions/
│   └── sess_<timestamp>_<hash>/
│       ├── manifest.yaml            # session 元信息、body_id、outcome
│       ├── events.jsonl             # 原始事件流
│       ├── raw/
│       │   └── events.mcap          # MCAP 录像（含帧、telemetry）
│       └── artifacts/
│           └── ...                  # frame/event/telemetry artifact
```

> 注意：默认 `practice.seekdb.enabled: false`，所以运行结束后 **只有 PracticeCatalog 有数据**，SeekDB 是空的。需要执行 `rosclaw practice ingest-seekdb` 才会把 episode / body_cognition 等蒸馏结果写进 SeekDB。
...
```bash
ls ~/.rosclaw/practice/runs/rh56_rps/sessions/sess_20260714T093707Z_477cb614c8f8/
head -3 ~/.rosclaw/practice/runs/rh56_rps/sessions/sess_20260714T093707Z_477cb614c8f8/events.jsonl
```
...
```bash
# 列出 topic
mcap info ~/.rosclaw/practice/runs/rh56_rps/sessions/sess_20260714T093707Z_477cb614c8f8/raw/events.mcap

# 导出某 topic
mcap cat ~/.rosclaw/practice/runs/rh56_rps/sessions/.../raw/events.mcap --topic /camera/camera/color/image_raw
```

### 5.3 帧图

frames 存在 artifact 目录下，通常是 JPEG/PNG 序列，可用任意图片查看器打开。
...
| `praxis_events` | 运行时事件（intent、state、command、feedback 等） |
| `body_cognition` | 身体认知（力基线、接触分布、热限等） |
...
- 想看 LeRobot/Parquet 导出：

```bash
${PYTHON} -m rosclaw.cli practice export \
  --session sess_20260714T093707Z_477cb614c8f8 \
  --episode ep_20260714T093707Z_c27576c6 \
  --format lerobot \
  --data-root ~/.rosclaw/practice/runs/rh56_rps
```
